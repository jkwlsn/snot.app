...
(async () => {
  ...
  // --- Configuration ---
  const symptomTypes = ["Sneezing", "Snotty nose", "Sore throat", "Coughing", "Itchy eyes", "Headaches", "Tiredness", "Shortness of breath", "Sinus pressure"];
  ...
  const pollenCurves = [
    ...
    { name: "birch_pollen",   peak: 105, width: 45, amplitude: 1.0, maxCount: 220 },
    { name: "grass_pollen",   peak: 165, width: 60, amplitude: 0.9, maxCount: 180 },
    { name: "mugwort_pollen", peak: 200, width: 40, amplitude: 0.7, maxCount: 110 },
    { name: "ragweed_pollen", peak: 240, width: 35, amplitude: 0.6, maxCount: 140 },
    { name: "olive_pollen",   peak: 150, width: 30, amplitude: 0.1, maxCount: 40 } // Low levels
  ];
  ...
  const getDailyPollenLevels = (dayOfYear) => {
    const levels = {};
    for (const type of pollenTypes) { levels[type] = 0; }

    for (const curve of pollenCurves) {
      const score = gaussian(dayOfYear, curve.peak, curve.width, curve.amplitude);
      const count = Math.round(score * curve.maxCount * (Math.random() * 0.4 + 0.8)); // Add 20% randomness
  ...
  const createSymptomRecord = (date, dayBaseSeverity, dailyPollenLevels) => {
    const randomType = symptomTypes[Math.floor(Math.random() * symptomTypes.length)];
    ...
    if (hour >= 6 && hour < 12) { finalSeverity += Math.random() * 1.5; } // Morning peak
    else if (hour >= 18 && hour < 22) { finalSeverity += Math.random() * 1; } // Evening peak
    finalSeverity += (Math.random() - 0.5) * 2; // Add more randomness
    const severity = Math.max(1, Math.min(5, Math.round(finalSeverity)));

    const location = { latitude: 52.52 + (Math.random() - 0.5) * 0.05, longitude: 13.40 + (Math.random() - 0.5) * 0.05 };
  ...
  try {
    ...
    console.log("Generating 1 year of realistic data with associated pollen levels...");
    const today = new Date();
    const oneYearAgo = new Date(new Date().setFullYear(today.getFullYear() - 1));
    const startDayOfYear = Math.floor((oneYearAgo - new Date(oneYearAgo.getFullYear(), 0, 0)) / (1000 * 60 * 60 * 24));
    ...
    for (let i = 0; i < 365; i++) {
        ...
        const totalPollenScore = Object.values(dailyPollenLevels).reduce((a, b) => a + b, 0) / 780; // Normalize score

        let dayBaseSeverity = 1 + totalPollenScore * 4;
        if (dayOfWeek === 0 || dayOfWeek === 6) { dayBaseSeverity += 0.3; }

        const maxSymptomsForDay = Math.round(totalPollenScore * 100);
        const numSymptomsToday = Math.round(maxSymptomsForDay * (Math.random() * 0.8 + 0.6));

        if (numSymptomsToday > 0) {
            for (let j = 0; j < numSymptomsToday; j++) {
                recordsToAdd.push(createSymptomRecord(new Date(day), dayBaseSeverity, dailyPollenLevels));
            }
        }
    }
    ...
    console.log(`Successfully generated and added ${recordsToAdd.length} realistic symptom records.`);
