...
export function useChartDataTransformer(
  symptomsGroupedByDay: Ref<Map<string, SymptomRecord[]>>,
  valueExtractor: (symptomsForDay: SymptomRecord[]) => number,
  label: string,
  backgroundColor: string,
): Ref<ChartData<"bar", { x: number; y: number }[]>> { // Change x type to number
  const chartData = computed<ChartData<"bar", { x: number; y: number }[]>>(() => { // Change x type to number
